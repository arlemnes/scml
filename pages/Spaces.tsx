...
const SpaceCard: React.FC<{ 
  ...
  const images = space.images && space.images.length > 0 ? space.images : ['https://via.placeholder.com/800x400?text=Sem+Foto'];
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden flex flex-col group hover:shadow-md transition-all duration-300">
      <div className="h-48 overflow-hidden relative bg-slate-100 group">
        ...
        {/* Status Badge */}
        <span className={`absolute top-4 right-4 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider shadow-sm z-10 ${
          space.active ? 'bg-white/90 text-brand-700 backdrop-blur-sm' : 'bg-red-500 text-white'
        }`}>
